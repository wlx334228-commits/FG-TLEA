function [Population,Elite,PPre,Pre] = ulOptimizerReal(Population,Elite,PPre,Pre)
...
%         Offspring = [Offspring,llOff];
    ...
    if Problem.gen < 3 || (mod(Problem.gen,3) ~= 0 && ~isequal(sort(unique(PPre.ulDecs)),sort(unique(Pre.ulDecs))))
        if Problem.gen == 1
            PPre = Population;
            Pre = Population;
...
%             Elite = getElite([Elite,llOff],Problem.Nu*Problem.Nl);
        ...
        [Offspring,Elite,calHvS] = multi_task(Offspring,Elite,Population)
%         calHV = HV(calHvS,Problem.optimum);'
        calHV = HV(calHvS,[-400,-400]);
    else
        PPre = Pre;
        Pre = Population;
        [Offspring,Elite,calHvS] = multi_task(Offspring,Elite,Population);
...
function [Offspring,Elite,calHv] = multi_task(Offspring,Elite,Population)

function nofinish = terminationCheck(Archive,G,t,form)
    ...
    if isempty(Archive) || isempty(Archive{1,10})
        nofinish = 1;
    else
        tau = size(Archive,2);
        mergePop = [];
        ...
        minHV = min(hv);
        if nargin<2 
            nofinish = ((maxHV - minHV) / (maxHV + minHV)>= 0.0001);
        else
%             nofinish = ((maxHV - minHV) / (maxHV + minHV)>= 0.0001) * (G<t); %epsilonU
            nofinish = ((maxHV - minHV) / (maxHV + minHV)>= 0.0001) || G < t;
        end
    end
end

...
%--------------------------------------------------------------------------
    ...
    methods(Access = ?GUI)
        ...
        function obj = module_test(GUI)
            ...
            % Initialization
            obj.cb_filter([],[],2);
            obj.app.listA(1).Value = 'BLEMO';
            obj.app.listA(2).Value = 'DS1';
            obj.cb_updateList([],[],1);
            obj.cb_updateList([],[],-1);
        end
    end
    methods(Access = private)
        ...
        function cb_start(obj,~,~)
            ...
            elseif strcmp(obj.app.buttonC(1).Text,'Continue')
                ...
                try
                    ...
                    for i = 1 : 2
                        ...
                        if i == 1
                            ...
                            PRO = feval(item.title.Text,'Nu',para{1},'Nl',para{2},'Mu',para{3},'Ml',para{4},'Du',para{5},'Dl',para{6},'maxFE',para{7},'parameter',para(8:end));
                        end
                    end
                catch err
                ...
                obj.app.menu((PRO.Mu<=1)+1).value = 1;
        ...
        function outputFcn(obj,Algorithm,Problem)
            obj.app.slider.Value = (Problem.ulFE+Problem.llFE)/max((Problem.ulFE+Problem.llFE),Problem.maxFE);
            obj.cb_slider();
        ...
        function cb_slider(obj,~,~,ax)
            if ~isempty(obj.app.dropD(1).Items)
                ...
                rate = (PRO.ulFE+PRO.llFE)/max(PRO.ulFE+PRO.llFE,PRO.maxFE);
                ...
                obj.app.labelC.Text = sprintf('%d evaluations',PRO.ulFE+PRO.llFE);
...
%                             Draw(ALG.Metric(obj.app.menu(1).string,10),'-k.','LineWidth',1.5,'MarkerSize',10,{'Number of function evaluations',obj.app.menu(1).string,[]});
                            data1 = ALG.Metric(obj.app.menu(1).string,10);
                            Draw(data1(3:end,:),'-k.','LineWidth',1.5,'MarkerSize',10,{'Number of function evaluations',obj.app.menu(1).string,[]});
        ...
        function cb_toolbutton3(obj,~,~)
            if ~isempty(obj.app.dropD(1).Items)
                obj.app.menu((obj.data{obj.app.dropD(1).Value,2}.Mu<=1)+1).show();
            end
        end
        ...
        function cb_dropdown1(obj,~,~)
            if ~isempty(obj.app.dropD(1).Items)
                ...
                if obj.data{obj.app.dropD(1).Value,2}.Mu > 1

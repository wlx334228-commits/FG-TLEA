function Offspring = OperatorDE(Parent1,Parent2,Parent3,flag,Parameter)
...
%--------------------------------------------------------------------------
    ...
    %% Polynomial mutation
    if flag == 'u'
        Lower = repmat(Problem.ulMin,N,1);
        Upper = repmat(Problem.ulMax,N,1);
    elseif flag == 'l'
        Lower = repmat(Problem.llMin,N,1);
        Upper = repmat(Problem.llMax,N,1);
    else
        Lower = [repmat(Problem.ulMin,N,1),repmat(Problem.llMin,N,1)];
        Upper = [repmat(Problem.ulMax,N,1),repmat(Problem.llMax,N,1)];
    end
    
    Site  = rand(N,D) < proM/(Problem.Du+Problem.Dl); 
    mu    = rand(N,D);
    temp  = Site & mu<=0.5;

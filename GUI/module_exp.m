...
%--------------------------------------------------------------------------
    ...
    methods(Access = ?GUI)
        ...
        function obj = module_exp(GUI)
            ...
            obj.app.toolC(3)   = GUI.APP([1 3],3,uibutton(tempGrid,'state','Text','Nu','BackgroundColor',[.95 .95 1],'Tooltip','Show the population size--upper level','ValueChangedFcn',@obj.TableUpdateColumn));
            obj.app.toolC(4)   = GUI.APP([1 3],4,uibutton(tempGrid,'state','Text','Nl','BackgroundColor',[.95 .95 1],'Tooltip','Show the population size--lower level','ValueChangedFcn',@obj.TableUpdateColumn));
            obj.app.toolC(5)   = GUI.APP([1 3],5,uibutton(tempGrid,'state','Text','maxFE','BackgroundColor',[.95 .95 1],'Tooltip','Show the maximum number of function evaluations','ValueChangedFcn',@obj.TableUpdateColumn));
            obj.app.toolC(6)   = GUI.APP([1 3],6,uibutton(tempGrid,'state','Text','D','BackgroundColor',[.95 .95 1],'Value',1,'Tooltip','Show the number of decision variables','ValueChangedFcn',@obj.TableUpdateColumn));
    ...
    methods(Access = private)
                    ...
                    for i = 1 : length(type)
                        ...
                        if type(i) > 0
                            ...
                            for j = 1 : max(1,max(len))
                                ...
                                PRO            = [PRO,feval(item.title.Text,'Nu',paraSub{1},'Nl',paraSub{2},'Mu',paraSub{3},'Ml',paraSub{4},'Du',paraSub{5},'Dl',paraSub{6},'maxFE',paraSub{7},'parameter',paraSub(8:end))];
                            end
                        end
                    end
                catch err
        ...
        function TableUpdateColumn(obj,~,~)
            if ~isempty(obj.data)
                ...
                str = arrayfun(@num2str,[obj.data.PRO.Nu;obj.data.PRO.Nl;obj.data.PRO.maxFE;obj.data.PRO.Du;obj.data.PRO.Dl]','UniformOutput',false);
        ...
        function TableUpdate(obj,~,~,proindex)
            ...
            if ~isempty(obj.data)
                ...
                obj.app.table.removeStyle();
                % Identify the metric
                metric = obj.app.dropC((obj.data.PRO(1).Mu<=1)+1).Value;
                    ...
                    for p = proindex
                        ...
                        for a = valid
                            ...
                            if ismember(obj.app.dropC(3).Value,[1 3])
                                ...
                                str = sprintf('%.2e (%.2e)',mdata(a),sdata(a));
                            end
        ...
        function ResultLoad(obj,p,a,r)
            try
                filename = fullfile(obj.data.folder,class(obj.data.ALG(a)),sprintf('%s_%s_%d.mat',class(obj.data.ALG(a)),class(obj.data.PRO(p)),r));
        ...
        function ResultSave(obj,p,a,r,result,metric)
            ...
            [~,~]    = mkdir(folder);
            filename = fullfile(folder,sprintf('%s_%s_%d.mat',class(obj.data.ALG(a)),class(obj.data.PRO(p)),r));
        ...
        function score = GetMetricValue(obj,p,a,metricName,showAll)
            ...
            for r = find(reshape(~cellfun(@isempty,obj.data.result(p,a,:)),1,[]))
                if ~isfield(obj.data.metric{p,a,r},metricName)
                    ...
                    save(fullfile(obj.data.folder,class(obj.data.ALG(a)),sprintf('%s_%s_%d.mat',class(obj.data.ALG(a)),class(obj.data.PRO(p)),r)),'metric','-append');
                end
                try
            ...
            if ~isempty(obj.app.table.Data)
                try
                    ...
                    if ischar(Name)
                        ...
                        switch Type
                            case '.xlsx'
                                table2excel(fullfile(Path,Name),obj.app.dropC((obj.data.PRO(1).Mu<=1)+1).Value,[{'Problem'},obj.app.table.ColumnName';obj.app.table.RowName,obj.app.table.Data],cat(1,obj.app.table.StyleConfigurations.TargetIndex{:}),size(obj.data.result,2));
                            case '.tex'
                        	...
                        	case '.mat'
                                table2mat(fullfile(Path,Name),obj.data.metric,obj.app.dropC((obj.data.PRO(1).Mu<=1)+1).Value);
                        end
                    end
                catch err
        ...
        function cb_tableSelect(obj,~,event)
            if ~isempty(obj.data)
                ...
                if ~isempty(grids) && ~ismember(obj.app.dropC((obj.data.PRO(1).Mu<=1)+1).Value,{'Number of runs','runtime'})
...
function score = Minimumvalue(Population,~)
    score = Population.ulBest.ulObjs;
